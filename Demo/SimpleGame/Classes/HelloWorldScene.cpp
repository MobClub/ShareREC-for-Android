...
{	
    ...
    CCARRAY_FOREACH(_projectiles, it)
	{
        ...
        CCARRAY_FOREACH(targetsToDelete, jt)
		{
			...
			// i increase the difficulty of the game, from 5 targets to 30
...
// add some menu items to show ShareRec operations
void HelloWorld::initShareRecMenuItems() {
	if (ShareRec::isAvailable()) {
		CCLabelTTF* lblStart = CCLabelTTF::create("Start Recorder", "Arial", 12, CCSizeZero, kCCTextAlignmentLeft, kCCVerticalTextAlignmentCenter);
		CCMenuItemLabel* startRec = CCMenuItemLabel::create(lblStart, this, menu_selector(HelloWorld::onStartRec));
		startRec->setColor(ccc3(0, 0, 0 ));

		CCLabelTTF* lblStop = CCLabelTTF::create("Stop Recorder", "Arial", 12, CCSizeZero, kCCTextAlignmentLeft, kCCVerticalTextAlignmentCenter);
		...
		itemsMenu->alignItemsVerticallyWithPadding(6);
		CCPoint origin = CCDirector::sharedDirector()->getVisibleOrigin();
		CCSize visibleSize = CCDirector::sharedDirector()->getVisibleSize();
		itemsMenu->setPosition(ccp(origin.x + 50, visibleSize.height - 15));
		this->addChild(itemsMenu);
	}
}

// this method will be called when ShareRec is stoping and stopped
...
// start recording
void HelloWorld::onStartRec(CCObject * pSender) {
	ShareRec::setOnRecorderStateListener(onStateChange);
